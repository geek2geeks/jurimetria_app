...
## ADR-06: Congelar caminhos referenciados pelo quickstart; não reorganizar a documentação por pastas
- **Data:** Junho de 2026.
- **Contexto:** O `quickstart.pdf` foi distribuído aos 8 membros (14/06/2026) e é um snapshot fora do nosso controlo. Aponta para os 8 guias via links GitHub `blob/main/<caminho>` e refere dezenas de caminhos em texto (`docs/...`, `src/dados/esquemas.py`, `data/README.md`). O GitHub serve sempre o conteúdo **atual** de cada `blob/main/<caminho>`, mas **não** redireciona ficheiros movidos ou renomeados (link antigo passa a 404).
- **Decisão:** Manter a estrutura de pastas atual — a separação por `docs/guias_individuais/` e `docs/especificacoes/` já é suficiente. A documentação atualiza-se **no sítio** (mesmo caminho); os ficheiros referenciados pelo quickstart não podem ser movidos, renomeados nem apagados enquanto o PDF distribuído estiver em vigor. A lista de caminhos congelados e o procedimento de atualização segura estão em `docs/convencoes_nomes.md` (secção "Estabilidade de caminhos").
- **Consequência:** Edições de conteúdo refletem-se automaticamente nos mesmos links, sem re-enviar nada. Qualquer reorganização ou renomeação futura obriga a re-emitir e redistribuir o quickstart (`python construir_quickstart_pdf.py`) e a atualizar este registo.
