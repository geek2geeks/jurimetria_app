...
> **Estás preso/a ou com dúvidas? Fala com o Gustavo (Scrum Master) ou Pedro (Tech Lead)** no grupo de WhatsApp / canal da equipa. A sério: **não há perguntas parvas** e perguntar cedo poupa horas a toda a gente. Estamos cá para nos ajudarmos uns aos outros.
...
### Gustavo — P3 · Limpeza e categorias / Scrum Master
...
- **1º passo:** validar as 5 classes (`MANTIDA`, `REVOGADA`, `ANULADA`, `NAO_CONHECIDA`, `OUTRA`) e gerir o fluxo do quadro Jira.
...
- **1º passo:** criar o vetorizador TF-IDF com matrizes NumPy e `fit` estrito apenas no treino.
...
- **1º passo:** montar a arquitetura `nn.Module` e ciclo de treino/validação.

### Luciana — P6 · Avaliação e métricas
...
- **1º passo:** implementar Macro-F1 e baseline da classe maioritária.
...
> **Toda a gente faz pelo menos um commit de código** — é critério de avaliação ("contribuições individuais"). Mesmo as tarefas mais de documentação têm uma pequena parte de código. E se isto parecer muito: respira, dá um passo de cada vez, e pergunta ao Gustavo ou Pedro. 🙂
...
**Encravaste na instalação? Fala com o Gustavo (Scrum Master) ou Pedro (Tech Lead).** É totalmente normal — cada máquina é diferente.
...
5. **Pull Request:** `git push -u origin a-tua-branch`, abre PR para `main`, liga o ticket, diz se usaste IA, e **pede revisão ao Gustavo (Scrum Master) e Pedro (Tech Lead)**.
6. **Done:** depois da aprovação e merge, move o ticket para **Done**. 🎉
...
|---|---|
| **Estou bloqueado / não percebo a tarefa** | Move o ticket Jira para **Blocked**, escreve o que tentaste, e **contacta o Gustavo (Scrum Master) ou Pedro (Tech Lead)**. Um bloqueio bem explicado é melhor do que esconder. |
...
| **Dúvida sobre conceitos/jargão** | Consulta o mini-glossário (secção 7) e os glossários em `docs/guia_iniciantes.md`. Continua sem perceber? **Pedro ou Gustavo.** |

> **Regra de ouro:** em caso de dúvida, **pergunta ao Gustavo (Scrum Master) ou Pedro (Tech Lead)** antes de fazeres algo de que não tens a certeza. Vale sempre a pena. 🙌
...
**Qualquer dúvida, em qualquer ponto: contacta o Gustavo (Scrum Master) ou Pedro (Tech Lead).** Conta connosco — e bom trabalho, equipa! 🚀
