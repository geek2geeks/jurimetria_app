# Onboarding — Gustavo (P3: Limpeza de Texto e Normalização de Labels)
...
```python
def clean_texto(texto: str) -> str:
    ...

def normalizar_label(decisao_raw: str | None) -> str | None:
    ...
```

## Por que isto é importante academicamente

A disciplina avalia pré-processamento e qualidade de pipeline. Em IA, os modelos não aprendem diretamente a realidade: aprendem padrões nos dados que lhes damos. Se o texto tiver rodapés, contactos do CSM, páginas, símbolos repetidos ou labels inconsistentes, o modelo aprende ruído.

A tua etapa mostra que Machine Learning depende fortemente da qualidade do pré-processamento. Também mostra que problemas reais raramente vêm prontos num CSV limpo.
...
Não alteres `schemas.py`. Não uses o campo `Decisão` como parte do texto de input do modelo. Não prometas que as heurísticas jurídicas são perfeitas; documenta que são uma primeira aproximação.
...
Pede um plano para regras pequenas e testáveis. Não deixes a IA inventar interpretações jurídicas: o mapa de labels foi aprovado pelo Pedro e casos ambíguos devem ser revistos. Usa exemplos sintéticos, executa os testes e declara o apoio de IA no PR.
