# Log Arquitetural de Decisões (ADR - Architectural Decision Records)
...
## ADR-02: Isolamento da Rota Analítica "Decisão" (Data Leakage Fix)
- **Data:** Junho de 2026.
...
- **Decisão:** `decisao_raw`, texto dispositivo, ECLI, URL e trechos de `full_text` que revelem a decisão ficam fora de X. Apenas `descritores` e `sumario` limpos podem alimentar o TF-IDF. A decisão normalizada segue separadamente como label Y.
